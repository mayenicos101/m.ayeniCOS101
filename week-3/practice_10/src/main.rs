fn main() {
    // addition
    let sum:i32 = 5550 + 7310;
    println!("The sum of 5550 and 7310 = {}", sum);

    // subtraction
    let difference:f64 = 95.5 - 4.3;
    println!("The difference of 95.5 adn 4.3 = {}", difference);

    // multiplication
    let product:f32 = 4.0 * 30.0;
    println!("The multiple of 4 and 30 = {}", product);

    //division
    let quotient:f64 = 56.70 / 32.20;
    println!("The division of 56.70 and 32.20 = {}", quotient);

    //remainder
    let remainder:i32 = 43 % 5;
    println!("The remainder when 43 is divided by 5 is {}", remainder);
}
